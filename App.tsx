...
const Sidebar: React.FC<{
...
}> = ({ activePage, onNavigate, onSignOut, onShowWhatsNew, chats, activeChatId, onNewChat, onSelectChat, onDeleteChat }) => {
    ...
    const NavLink: React.FC<{
        ...
        <a
            ...
            className={`flex items-center px-3 py-2.5 rounded-lg transition-all duration-200 group relative ${activePage === item.page ? 'text-purple-700 dark:text-purple-300' : 'text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800/60 dark:hover:text-white'}`}
        >
             <div className={`absolute left-0 top-1/2 -translate-y-1/2 h-6 w-1 bg-purple-500 rounded-r-full transition-opacity duration-300 ${activePage === item.page ? 'opacity-100' : 'opacity-0'}`}></div>
             <div className="relative flex items-center">
                 <item.icon className={`h-5 w-5 ${activePage === item.page ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400 group-hover:text-gray-500 dark:group-hover:text-white'}`} />
                <span className={`ml-4 font-medium text-sm ${activePage === item.page ? 'font-semibold' : ''}`}>{item.name}</span>
            </div>
...
const App: React.FC = () => {
    ...
    return (
        <div className="h-full w-full flex flex-col bg-gray-100 dark:bg-[#111827] text-gray-800 dark:text-gray-200">
