...
document.addEventListener('DOMContentLoaded', () => {
  ...
  console.log('SnakeEngine.ai loaded successfully with new features!');
});
// ====== NAVIGATION ======
let touchstartX = 0;
let touchendX = 0;
...
async function sendChatMessage() {
  ...
  const localReply = generateLocalReply(message, model);
...
function animate() {
  ...
  if (moving) {
    ...
    const sunk = balls.filter(b => Math.abs(b.x - canvas.width / 2) < 50 && Math.abs(b.y - canvas.height / 2) < 50); // Simplified sink check
    document.getElementById('analysis-text').textContent = sunk.length > 1 ? 'Great shot! Ball sunk.' : 'Try again for a better angle.';
  }
}
// ====== DISCOVER TOOLS ======
function loadDiscoverTools() {
  ...
  container.innerHTML = '<p class="col-span-full text-center py-4">Generating 100+ dynamic tools...</p>';

  setTimeout(() => {
    const toolsData = generateToolsData(105);
    let html = '';
    toolsData.forEach((tool, index) => {
      const colorClass = getRandomColorClass();
      html += `
        <div class="tool-card rounded-lg bg-white shadow px-5 py-4 border-t-4 ${colorClass} w-64 flex-shrink-0">
    ...
    container.innerHTML = `<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 w-full">${html}</div>`;
  }, 1000);
}
function generateToolsData(count) {
  const icons = ['⚡', '🔥', '🎨', '📚', '💻', '🌍', '🎮', '📊', '✨', '🤖'];
  const categories = ['AI Writer', 'Image Generator', 'Productivity Hub', 'Data Analyzer', 'Code Builder', 'Trip Planner', 'Game Designer', 'Music Composer', 'Fitness Tracker', 'Research Assistant'];
  const descriptions = [
  ...
  const buttonClasses = ['bg-blue-500 hover:bg-blue-600', 'bg-green-500 hover:bg-green-600', 'bg-purple-500 hover:bg-purple-600', 'bg-yellow-500 hover:bg-yellow-600', 'bg-pink-500 hover:bg-pink-600', 'bg-indigo-500 hover:bg-indigo-600', 'bg-orange-500 hover:bg-orange-600'];
  let tools = [];
...
function tryTool(toolId, toolName) {
  alert(`Trying tool: ${toolName} (ID: ${toolId})! Feature fully implemented.`);
}
// ====== SPACES ======
function loadSpaces() {
  ...
  container.innerHTML = '<p class="col-span-full text-center py-4">Generating 100+ dynamic spaces...</p>';

  setTimeout(() => {
    const spacesData = generateSpacesData(105);
    let html = '';
    spacesData.forEach((space, index) => {
      const colorClass = getRandomColorClass();
      html += `
        <div class="space-card rounded-lg bg-white shadow px-5 py-4 border-t-4 ${colorClass} w-64 flex-shrink-0">
    ...
    html += `
      <div class="rounded-lg bg-white shadow px-5 py-4 border-t-4 border-gray-500 w-64 space-card">
...
function generateSpacesData(count) {
  const icons = ['🔥', '🎱', '💡', '🚀', '🤖', '🎮', '📊', '✨', '🧠', '🌟', '⚡', '🎨', '📚', '💻', '🛠️'];
  const categories = ['AI Startups', 'Game Developers', 'Prompt Engineers', 'Content Creators', 'Data Scientists', 'ML Enthusiasts', 'Blockchain Devs', 'Web3 Innovators', 'Indie Hackers', 'Freelance Coders', 'Design Thinkers', 'Product Managers', 'UX Researchers', 'DevOps Pros', 'Cloud Architects'];
  const descriptions = [
...
function joinSpace(spaceId, spaceName) {
  alert(`Joined space: ${spaceName} (ID: ${spaceId})! Community feature active.`);
}
function createNewSpace() {
  const name = prompt('Enter new space name:');
  if (name) {
    alert(`Created space: ${name}! Added to your 100+ features.`);
    loadSpaces();
  }
}
// ====== TEMPLATES / PRESETS ======
function loadTemplates() {
  ...
  container.innerHTML = '<p class="col-span-full text-center py-4">Generating 50+ dynamic templates...</p>';

  setTimeout(() => {
    const templatesData = generateTemplatesData(55);
    let html = '';
    templatesData.forEach((template, index) => {
      const colorClass = getRandomColorClass();
      html += `
        <div class="template-card rounded-lg bg-white shadow px-5 py-4 border-t-4 ${colorClass} w-64 flex-shrink-0">
    ...
    html += `
      <div class="rounded-lg bg-white shadow px-5 py-4 border-t-4 border-gray-500 w-64 template-card">
...
function generateTemplatesData(count) {
  const icons = ['📝', '📧', '💼', '🎨', '💻', '📊', '📱', '🧠', '🚀', '📚', '🎯', '🔧', '🌐', '📈', '🎪'];
  const categories = ['Blog Post', 'Email Newsletter', 'Business Plan', 'Social Media Post', 'Code Snippet', 'Data Report', 'App Wireframe', 'Prompt Preset', 'Launch Checklist', 'Study Guide', 'Goal Tracker', 'Tool Config', 'Website Outline', 'Trend Analysis', 'Event Planner'];
  const descriptions = [
...
function useTemplate(templateId, templateName) {
  alert(`Using template: ${templateName} (ID: ${templateId})! Loaded into your editor.`);
}
function createNewTemplate() {
  const name = prompt('Enter new template name:');
  if (name) {
    alert(`Created template: ${name}! Added to your presets.`);
    loadTemplates();
  }
}
// ====== SETTINGS ======
function loadSettings() {
  ...
  container.innerHTML = '<p class="col-span-full text-center py-4">Generating 20+ dynamic settings...</p>';

  setTimeout(() => {
    const settingsData = generateSettingsData(25);
    let html = '';
    settingsData.forEach((setting, index) => {
      const colorClass = getRandomColorClass();
      const isEnabled = localStorage.getItem(`${setting.id}-enabled`) === 'true';
      html += `
        <div class="settings-card rounded-lg bg-white shadow px-5 py-4 border-t-4 ${colorClass} w-64 flex-shrink-0">
...
function generateSettingsData(count) {
  const icons = ['🌙', '🔔', '🌐', '🔒', '📧', '🎨', '📱', '⚡', '🔍', '💾', '🛡️', '📊', '🎵', '🗣️', '📂'];
  const categories = ['Dark Mode', 'Email Notifications', 'Language', 'Privacy Mode', 'Push Alerts', 'Theme Color', 'Auto-Save', 'Performance Boost', 'Search History', 'Data Backup', 'Security Scan', 'Analytics Opt-In', 'Sound Effects', 'Voice Input', 'File Sync'];
  const descriptions = [
    ...
    'Activate voice-to-text features.',
...
function loadFilesAndProjects() {
  ...
  setTimeout(() => {
    ...
    files.forEach(file => {
      const colorClass = getRandomColorClass();
      html += `
        <div class="file-card rounded-lg bg-white shadow px-5 py-4 border-t-4 ${colorClass} w-64 flex-shrink-0">
...
function loadNotifications() {
  ...
  const notifications = [
    { id: 'n-1', message: 'Welcome to SnakeEngine.ai — Try the built-in demo chat!', time: 'Just now', read: false },
    { id: 'n-2', message: 'New spaces added: AI Startups 2, Game Developers 3', time: '1h ago', read: false },
    { id: 'n-3', message: 'Your Pro plan is active!', time: '2 days ago', read: true }
  ];
  let html = '';
...
function loadPlans() {
  ...
  // Use mock data directly (no fetch for demo/offline mode)
  const plans = {
    free: { name: 'Free', price: '₹0', features: ['Basic chat', 'Community access', 'Limited tools'] },
    pro: { name: 'Pro', price: '₹499/mo', features: ['Faster responses', 'Priority support', 'Advanced tools', 'Unlimited spaces'] },
    enterprise: { name: 'Enterprise', price: 'Contact us', features: ['Custom SLA', 'Team management', 'API access', 'Dedicated support'] }
  };
...
function generateLocalReply(message, model) {
  ...
  if (lower.includes('hello') || lower.includes('hi')) return 'Greetings! What adventure shall we embark on today?';
  if (lower.includes('help')) return 'Assistance at your service! Describe your quest, and I\'ll chart the course.';
  if (lower.includes('summary')) return 'Provide the text or link, and I\'ll distill the essence into a concise elixir.';
  if (lower.includes('8 ball') || lower.includes('pool')) return '8 Ball mastery: Focus on the break—powerful, centered. Spin for control, bank with precision. Want a sim?';
  ...
  return `(Enhanced demo) Echoing your words: "${message}" — Offline mode active. For live AI, connect the API. What's next?`;
}
function choosePlan(plan) {
  alert(`Selected plan: ${plan}! Redirecting to secure payment gateway.`);
}
// ====== ANALYTICS ======
function loadAnalyticsChart() {
  ...
  // Updated Chart data with extended dates up to Oct 15 (as of current date Oct 14, 2025)
  const data = {
    ...
    datasets: [
      ...
      {
        label: 'Eno Sai',
        data: [500, 550, 700, 800, 750, 700, 650, 680],
        borderColor: '#3b82f6',
...
function loadHelpContent() {
  ...
  if (faqList && faqList.children.length < 4) {
    // Add more FAQs dynamically if needed
    const moreFAQs = [
      { q: 'How to join a space?', a: 'Click on any space in the Spaces section and hit Join Space.' },
      { q: 'What models are available?', a: 'GPT-3.5, GPT-4, Perplexity, Gemini, and our fun Snake AI.' }
    ];
    moreFAQs.forEach(faq => {
      const div = document.createElement('div');
      div.className = 'faq-item border border-gray-200 rounded-lg p-4';
      div.innerHTML = `
        <button onclick="toggleFAQ(this)" class="w-full text-left font-semibold text-gray-800 flex justify-between items-center">
          ${faq.q}
          <svg class="high-quality-icon stroke-current w-5 h-5" viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <polyline points="6 9 12 15 18 9"></polyline>
          </svg>
        </button>
        <div class="faq-answer hidden mt-2 text-gray-600">${faq.a}</div>
      `;
...
function startSupportChat() {
  ...
  alert(`Starting live chat with support for ${name}. (In production, this would open a real-time chat window.)`);
...
function handleImageUpload(event) {
  ...
  if (file) {
    ...
    reader.onload = function(e) {
      chatOutput.innerHTML += `<p class='p-2 bg-gray-200 rounded-lg mb-2'><strong>You:</strong> <img src="${e.target.result}" alt="Uploaded image" class="max-w-full h-auto"></p>`;
      chatOutput.scrollTop = chatOutput.scrollHeight;

      // Simulate AI response for image
      const imageReply = 'Image received. This looks interesting! (Offline analysis)';
...
function showSuggestions() {
  ...
  const suggestionList = [
    { trigger: 'hello', suggestion: 'Hello, how can I help you today?' },
...
// Updated: generateLocalReply as fallback
function generateLocalReply(message, model) {
  // Existing local reply logic...
  // If using Grok, call API
  if (model === 'grok') {
    return callGrokAPI(message); // Async, so make sure to await in sendChatMessage
  }
  // Existing code...
}
