...
const ThinkingModeTool: React.FC<ToolProps> = ({ onBack }) => {
    ...
    return (
        <div className="h-full flex flex-col p-4 md:p-8 bg-transparent overflow-y-auto">
            ...
            <div className="text-center mb-4">
                <h1 className="text-3xl md:text-4xl font-bold text-gray-800 dark:text-white">Thinking Mode</h1>
                <p className="text-gray-500 dark:text-gray-400 mt-2">For deep, complex, and nuanced conversations.</p>
            </div>
                    ...
                    <div className="flex flex-col space-y-4">
                        {messages.map((msg, index) => (
                            <div key={index} className={`flex items-start gap-3 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                                ...
                                <div className={`max-w-md px-4 py-2 rounded-2xl ${msg.sender === 'user' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-bl-none'}`}>
                                    {msg.isThinking ? (
                ...
                <div className="p-4 border-t border-gray-200 dark:border-gray-700">
                    <form onSubmit={handleSendMessage} className="relative">
                        <input
                            ...
                            className="w-full py-3 pl-4 pr-16 rounded-full bg-gray-100 dark:bg-gray-900/50 focus:outline-none focus:ring-2 focus:ring-purple-500 transition"
                            disabled={isLoading}
                        />
