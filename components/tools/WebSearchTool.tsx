...
interface WebSearchToolProps { onBack: () => void; }
...
const WebSearchTool: React.FC<WebSearchToolProps> = ({ onBack }) => {
        ...
        <div className="h-full flex flex-col p-4 md:p-8 bg-gray-50">
            ...
            <div className="w-full max-w-4xl mx-auto flex-grow flex flex-col">
                <div className="text-center mb-8">
                    <h1 className="text-3xl md:text-4xl font-bold text-gray-800">Web Search</h1>
                    <p className="text-gray-500 mt-2">Get up-to-date, real-world information grounded in Google Search.</p>
                </div>

                <div className="bg-white p-6 rounded-2xl shadow-lg">
                    <div className="flex items-center gap-4">
                        <input value={prompt} onChange={(e) => setPrompt(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch()} placeholder="e.g., Who won the most recent F1 race?" className="w-full p-3 bg-gray-100 rounded-lg border border-transparent focus:outline-none focus:ring-2 focus:ring-purple-400 transition" disabled={isLoading} />
                ...
                <div className="mt-8 flex-grow">
                    {isLoading && <div className="flex justify-center items-center h-full"><div className="text-center text-gray-500"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-500 mx-auto"></div><p className="mt-4">Searching the web...</p></div></div>}
                    {result && (
                        <div className="bg-white p-6 rounded-2xl shadow-lg">
                            ...
                            {result.sources && result.sources.length > 0 && (
                                <div className="mt-6 pt-4 border-t">
                                    <h4 className="font-semibold text-gray-600 mb-2">Sources:</h4>
