...
import { generateNotionContent } from '../../services/geminiService';
import { NotionIcon } from '../../constants';

const Spinner = () => <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>;

interface ToolProps { onBack: () => void; }

const NotionTool: React.FC<ToolProps> = ({ onBack }) => {
    const [prompt, setPrompt] = useState('');
    const [isLoading, setIsLoading] = useState(false);
    const [result, setResult] = useState<{ content: string | null, error: string | null }>({ content: null, error: null });

    const handleGenerate = async () => {
        if (!prompt.trim() || isLoading) return;
        setIsLoading(true);
        setResult({ content: null, error: null });
        const response = await generateNotionContent(prompt);
        setResult(response);
        setIsLoading(false);
    };
        ...
        <div className="h-full flex flex-col p-4 md:p-8 bg-transparent overflow-y-auto">
            <div className="w-full max-w-4xl mx-auto flex-grow flex flex-col">
                ...
                <div className="text-center mb-8">
                    <NotionIcon className="h-12 w-12 mx-auto" />
                    <h1 className="text-3xl md:text-4xl font-bold text-gray-800 dark:text-white mt-2">Notion Assistant</h1>
                    <p className="text-gray-500 dark:text-gray-400 mt-2">Generate structured content for your knowledge base.</p>
                </div>
                ...
                <div className="bg-white/80 dark:bg-gray-800/60 backdrop-blur-xl p-6 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
                    <textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="What content do you need? e.g., 'Create a meeting agenda for the Q4 planning session.'" className="w-full p-3 h-24 bg-gray-100 dark:bg-gray-700 rounded-lg border border-transparent focus:outline-none focus:ring-2 focus:ring-purple-500" />
                ...
                <div className="mt-8 flex-grow">
                    ...
                    {result.content && (
                        <div className="bg-white/80 dark:bg-gray-800/60 backdrop-blur-xl p-6 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
                             <h2 className="text-xl font-bold mb-4 text-gray-800 dark:text-white">Generated Content</h2>
